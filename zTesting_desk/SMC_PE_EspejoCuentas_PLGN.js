        ...
        for (var key in jsonAccounting) {
            ...
            if (jsonAccounting[key].length && trBook && trBook == currentBook) {
                ...
                for (var j = 0; j < jsonAccounting[key].length; j++) {
                    nlapiLogExecution("DEBUG", "jsonAccounting[key][j]", jsonAccounting[key][j]);
                    ...
                    if (JsonItem[jsonAccounting[key][j]["item"]]) {
                    ...
                    if (!jsonCuentas.hasOwnProperty(accountKey)) {
                        nlapiLogExecution("DEBUG", "jsonCuentas", jsonCuentas);
                        nlapiLogExecution("DEBUG", "accountKey", accountKey);
                        continue;
                    }
