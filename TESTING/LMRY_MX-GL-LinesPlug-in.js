		...
		for (var countGLNew = 0; countGLNew < standardLines.getCount(); countGLNew++) {
			...
			for(var cuentaNew=0; cuentaNew<searchresult.length; cuentaNew++)
			{
				...
				if(cuentaOrigenStandard == AccouMappingOrigen)
				{
					...
					nlapiLogExecution('EMERGENCY', 'DueDateRecord' , (nlapiStringToDate(DueDateRecord,'date')-nlapiStringToDate(TranDateRecord,'date')));
					...
					if (lmryTypeTransaction==7)
					{
							...
							(EsRelacionada=='F' || EsRelacionada=='f') && (EsExtranjero=='F' || EsExtranjero =='f'))
						{
							...
							nlapiLogExecution('EMERGENCY', 'set account' , "set account");
							nlapiLogExecution('EMERGENCY', 'set account recaccfr' , recaccfr);
							nlapiLogExecution('EMERGENCY', 'set account recaccto' , recaccto);
						}
					...
					if (recaccto!='' && recaccfr!='')
					{
						...
						nlapiLogExecution('EMERGENCY', 'feamultibook' , feamultibook);
						nlapiLogExecution('EMERGENCY', 'feaaccountMapping' , featAccountMapping);
