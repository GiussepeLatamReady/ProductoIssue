 ...
 * @NModuleScope Public
 */
define(['N/url', 'N/record', 'N/currentRecord', 'N/ui/dialog', 'N/runtime','N/search', 
  ...
  function(url, record, currentRecord, dialog, runtime, search, library, libLog) {
    ...
    function saveRecord(scriptContext) {
      ...
      try {
        ...
        var arrayEdited = [];
        var jsonPaymentRet = {};
        //Array para Serie y Comprobante
        var arrayInvoicing = {};
        ...
        if(row > 0){
          for(var i = 0; i < row; i++){
            ...
            var serieInv = objRecord.getSublistValue({sublistId:'custpage_id_sub',fieldId:'sub_serie_inv',line:i}) || '';
            var numComprobante = objRecord.getSublistValue({sublistId:'custpage_id_sub',fieldId:'sub_comprobante',line:i}) || '';
            var nationalContributory = objRecord.getSublistValue({sublistId:'custpage_id_sub',fieldId:'sub_tip_ret',line:i}) || '';
            var natxcont = objRecord.getSublistValue({sublistId:'custpage_id_sub',fieldId:'sub_by_subsidiary',line:i});

            var invoice = objRecord.getSublistValue({sublistId: 'custpage_id_sub',fieldId: 'sub_transa',line: i});
            nationalContributory = nationalContributory.match(/>(\d+)</)[1];
            ...
            if(!check){
              ...
              if (!arrayInvoicing[invoice]) {
                arrayInvoicing[invoice] = {};
              }
              
              if(nationalContributory){
                if(natxcont=='True' || natxcont ==true){
                  nationalContributory='NT'+nationalContributory;
                }else{
                  nationalContributory='CC'+nationalContributory;
                }
                arrayInvoicing[invoice][nationalContributory]=({'invserie': serieInv, 'comprobante': numComprobante });
              }
        ...
        record.submitFields({
          type: 'customrecord_lmry_new_latamtax_sales_log',
          id: idLog,
          values: {
            custrecord_lmry_new_latamtax_noretention: JSON.stringify(arrayNoRet),
            custrecord_lmry_new_latamtax_edited: JSON.stringify(arrayEdited),
            custrecord_lmry_new_latamtax_serie_comp: JSON.stringify(arrayInvoicing)
          }
        });
