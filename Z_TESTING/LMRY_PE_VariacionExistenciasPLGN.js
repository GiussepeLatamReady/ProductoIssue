    ...
    try {
        ...
        if (country == 174) {//Peru
            ...
            if (FEAT_SUBSI == true || FEAT_SUBSI == "T") {
                subsidiarySetup = transactionRecord.getFieldValue("subsidiary");
            } else {
                subsidiarySetup = nlapiLoadConfiguration('companyinformation');
            }

            var processWithoutPO = validatePluginNoPO(subsidiarySetup);
            ...
            if ((type == 'vendorbill' || type == 'vendorcredit') && !processWithoutPO ) {
                var hasPOs = false;
                ...
                for (var i = 1; i <= numItems; i++) {
                    ...
                    var orderdoc = transactionRecord.getLineItemValue("item", "orderdoc", i);
                    nlapiLogExecution("DEBUG", "orderdoc", orderdoc);
                    if ((orderdoc != '' && orderdoc != null) ) {
                        hasPOs = true;
                        break;
                    }
                }
            ...
            if (type == "vendorbill" || type == "vendorcredit") {
                for (var i = 1; i <= numItems; i++) {
                    //Para comprobar que los items del Vendor Bill O Bill Credit proviene del Purchase Order
                    var orderdoc = transactionRecord.getLineItemValue("item", "orderdoc", i);
                    if ((orderdoc != '' && orderdoc != null) || processWithoutPO) {
                ...
                for (var i = 1; i <= numItems; i++) {
                    ...
                    var class_ = transactionRecord.getLineItemValue("item", "class", i) || "";
                    var location = transactionRecord.getLineItemValue("item", "location", i) || "";
                    var landedcost = transactionRecord.getLineItemValue("item", "custcol_lmry_br_taxc_rsp", i) || "";
                    var parsedCost = landedcost ? JSON.parse(landedcost) : {};
                    if (parsedCost && Number(parsedCost.amount)) {
                        amount += Number(parsedCost.amount);
                    }
            ...
            if (type == "itemreceipt") {
                setLandedCostLine(transactionRecord, book, customLines, Json_GMA, Json_IMA, exchangeRate, accounts69Array)
            }
                ...
                if (defaultAccount) {
                    for (var i = 0; i < lines.length; i++) {
                        ...
                        if (amount && factNoEmitAccount) {
                            ...
                            var pbFactNoEmitAccount = factNoEmitAccount;
                            var pbDefaultAccount = defaultAccount;
                            ...
                            if (type == "vendorbill") {
                                ...
                                lineDebit.primaryBookAccount = pbFactNoEmitAccount;
                                lineCredit.account = defaultAccount;
                                lineCredit.primaryBookAccount = pbDefaultAccount;
                                ...
                                lineDebit.primaryBookAccount = pbDefaultAccount;
                                lineCredit.account = factNoEmitAccount;
                                lineCredit.primaryBookAccount = pbFactNoEmitAccount;
                            }
                    ...
                    for (var i = 0; i < lines.length; i++) {
                        ...
                        if (amount) {
                            if (cuentaVarExist) {
                                ...
                                var pbCuentaVarExist = cuentaVarExist;
                                var pbDefaultAccount = defaultAccount;
                                ...
                                if (type == "itemreceipt") {
                                    ...
                                    lineCredit.primaryBookAccount = pbCuentaVarExist;
                                    ...
                                    lineDebit.primaryBookAccount = pbCuentaVarExist;
                                    lineCredit.account = defaultAccount;
                                    lineCredit.primaryBookAccount = pbDefaultAccount;
                                }
                            ...
                            if (cuentaCompras && factNoEmitAccount) {
                                ...
                                var pbCuentaCompras = cuentaCompras;
                                var pbFactNoEmitAccount = factNoEmitAccount;
                                ...
                                if (type == "itemreceipt") {
                                    ...
                                    lineDebit.primaryBookAccount = pbCuentaCompras;
                                    lineCredit.account = factNoEmitAccount;
                                    lineCredit.primaryBookAccount = pbFactNoEmitAccount;
                                } else if (type == "itemfulfillment") {
                                    ...
                                    lineCredit.primaryBookAccount = pbCuentaCompras;
                                }
...
function setLandedCostLine(transactionRecord, book, customLines, Json_GMA, Json_IMA, exchangeRate, accounts69Array) {
    var numItems = transactionRecord.getLineItemCount("item");
    // Iteracion Item line
    for (var i = 1; i <= numItems; i++) {
        var itemId   = transactionRecord.getLineItemValue("item", "item", i);
        ...
        var class_   = transactionRecord.getLineItemValue("item", "class", i) || "";
        var location = transactionRecord.getLineItemValue("item", "location", i) || "";
        var unparsedLandedCost = transactionRecord.getLineItemValue("item", "custcol_lmry_br_taxc_rsp", i) || "";
        var dataLine = unparsedLandedCost ? JSON.parse(unparsedLandedCost) : {};

        if (Object.keys(dataLine).length) {                   
            for (var j = 0; j < dataLine.landedcost.length; j++) {
                // Landed Cost : Linea 
                var landedAmount = dataLine.landedcost[j].amount || 0;
                ...
                nlapiLogExecution('DEBUG', 'landedCost : Account', 
                                    'accocredit : ' + dataLine.landedcost[j].credit + 
                                    ', accondebit : ' + dataLine.landedcost[j].debit + 
                                    ', amount : ' + landedAmount);

                // Landed Cost : Account Credit & Debit
                if (dataLine.landedcost[j].credit!='' && dataLine.landedcost[j].debit!=''){
                    var lcAccCre = dataLine.landedcost[j].credit;
                    var lcAccDeb = dataLine.landedcost[j].debit;
                    ...
                    newCreditLandedCost.setCreditAmount(parseFloat(landedAmount));
                    ...
                    newDebitLandedCost.setDebitAmount(parseFloat(landedAmount));
                    ...
                    lineDebit.columna = "debit";
                    lineCredit.columna = "credit";
                    lineDebit.account = setAccDeb;
                    lineDebit.primaryBookAccount = lcAccDeb;
                    lineCredit.account = setAccCre;
                    lineCredit.primaryBookAccount = lcAccCre;
