     ...
     function calcularIGV(recordObj) {
         try {
            log.error("[calcular IGV] ","start")
             ...
             log.error("[calcular IGV] applywht",applywht)
             if (applywht == 'T' || applywht == true) {
                 ...
                 if (creditSearchObj != null && creditSearchObj != '') {
                     log.error("[calcular IGV] creditSearchObj",creditSearchObj[0])
                 ...
                 log.error("[calcular IGV] igvAccountOrig",igvAccountOrig)
                 var exchangeRate = 1;
                 ...
                 log.error("[calcular IGV] newExchangeRate",newExchangeRate);
                 var idJournal = 0;
                 var JEamount = 0;
                 ...
                 if (concepDetr == 354) {
                     ...
                     log.error("[calcular IGV] exchangeRate",exchangeRate);
                     log.error("[calcular IGV] igvPercent",igvPercent);
                     ...
                     log.error("[calcular IGV] JEamount",JEamount);
                     ...
                     log.error("[calcular IGV]  expense JEamount",JEamount);
                     rcdTransaction.save({
