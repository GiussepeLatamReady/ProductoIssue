  ...
  function (serverWidget, search, record, log, url, format, runtime, library_send, lbryLog, query) {
    ...
    function iteracion(script, bySubsidiary, result, columnas, porcet, posicion, tramo, vendor, orden, docFiscal, exchange_global, manual, noretention, suma, vendorvalue, pagoDeBill, jsonRaCC, jsonRaNT, isMultiCuenta, whtGroupByVendorActive) {
      function cleanName(str) {
        if (!str) return "";
        return str.replace(/[|]/g, "").replace(/[^a-zA-Z0-9\s\-\_]/g, "");
      }
        ...
        for (var k = 0; k < result.length; k++) {
          ...
          if (casoActual == "2") {
            ...
            if (val > 0) {
              ...
              arregloAuxiliar[contadorAuxiliar] += "|" + cleanName(tran_number[posicion]);
            ...
            if (val > 0) {
              ...
              log.error("orden_pre",orden_pre)
              log.error("0 tramo[0]",tramo[0])
              log.error("1 vendor",vendor)
              log.error("2 internalid",internalid)
              log.error("3 round(tax_rate,6)",round(tax_rate,6))
              log.error("4 ratio",ratio)
              log.error("5 ",parseFloat((parseFloat(aux_amount_remaining) * parseFloat(exchange_global)).toFixed(4)))
              log.error("6 ratio",parseFloat(Math.round(parseFloat(val) * 10000) / 10000))
              log.error("7 jurisdiccion_iib",jurisdiccion_iib)
              log.error("8 tran_number[posicion]",tran_number[posicion])
              log.error("9 ccl_apli",ccl_apli)
              arregloAuxiliar[contadorAuxiliar] = tramo[0]; //0
              arregloAuxiliar[contadorAuxiliar] += "|" + vendor;
              arregloAuxiliar[contadorAuxiliar] += "|" + internalid;
              arregloAuxiliar[contadorAuxiliar] += "|" + round(tax_rate,6);
