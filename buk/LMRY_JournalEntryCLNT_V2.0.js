  ...
  function (log, search, runtime, email, format, currentRecord, url, https, library, library_translator, record, Library_ExchangeRate, LR_webService) {
    ...
    function pageInit(scriptContext) {
      ...
      try {
        ...
        var subsidiary = urlObject.searchParams.get('subsidiary');
        var bookje = urlObject.searchParams.get('bookje');
         
        if (bookje) {
          var accountingBook = urlObject.searchParams.get('accountingBook');
          if (accountingBook) recordObj.setValue({ fieldId: 'accountingBook', value: accountingBook });
        } else {
          if (subsidiary) {
            changeSubsidiary = 1;
      ...
      try {
        ...
        if (name == "custbody_lmry_subsidiary_country") {
          ...
          if (subsi_OW == true || subsi_OW == "T") {
            ...
            ValidateAccessJE(subsidiaryID);

            if (changeSubsidiary>1) redirectToPageWithParameter(subsidiaryID,recordObj);
            changeSubsidiary++;
          } else {
            ValidateAccessJE(1);
          }
      ...
      try {
        ...
        if((library.getAuthorization(151, licenses) == true || library.getAuthorization(151, licenses) == 'T') && LMRY_countr[0] == 'MX'){
          ...
          for(var i = 0; i < lines; i++){
            ...
            var entityName = current.getSublistValue({
                sublistId: 'line',
            ...
            if (Number(entity) && entity != null && entity != '' && debit != null && debit != '') {
          ...
          if (Object.keys(entities).length > 0) {
              var checkEntity = search.create({
                ...
                filters: [
                    ['internalid', 'anyof', Object.keys(entities)]
                ]
            });
            ...
            checkEntity.run().each(function(result){
                ...
                var idEntity = result.getValue('internalid');
                var entityName = entities[idEntity]["entityName"];
                var entityCountry = result.getValue('custentity_lmry_countrycode');

                if(entityType != 'Vendor'){
                  errorDiot = errorDiot + ' — '+ entityName +'\n';
                  diotFail = true;
                }else{
                  if(opType){
                    if(!(entityCountry == 'MX' || entityCountry == '') && !(opCode== '03' || opCode =='85')){
                      errorExtranjero = errorExtranjero + ' — ' + entityName +'\n';
                      diotFail2 = true;
                    }
                  }
                }
    ...
    function redirectToPageWithParameter(subsidiaryID,recordObj) {
      var currentUrl = window.location.href;
      var bookje =  new URL(currentUrl).searchParams.get('bookje');
      var recall =  new URL(currentUrl).searchParams.get('recall');
      var account =  new URL(currentUrl).searchParams.get('account');
      var amount =  new URL(currentUrl).searchParams.get('amount');
      var paramDate =  new URL(currentUrl).searchParams.get('paramDate');
      if (recall) return false;
      if (account && amount && paramDate) return false;
      ...
      if (bookje) {
        var accountingBook = recordObj.getValue({ fieldId: "accountingBook" });
        currentUrl+="&accountingBook="+accountingBook;
      }
      window.onbeforeunload = null;
      window.location.href = currentUrl + "&recall=T";
    }
