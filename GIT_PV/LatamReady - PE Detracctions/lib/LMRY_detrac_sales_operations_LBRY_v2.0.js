 ...
 * @NApiVersion 2.1
 * @NModuleScope Public
 * @Name LMRY_detrac_sales_operations_LBRY_v2.0.js
 */
/*****************************************************************************
  VERSION 1.0
...
define(['N/error', 'N/log', 'N/runtime', 'N/search', 'N/url', 'N/config', 'N/format'],
  ...
  function (error, log, runtime, search, url, config, format) {
    ...
    function getSearchAccountBySubsidiary(subsidiaries){
      let resultContext = [];
      let searchContext = search.create({
        ...
        filters: [
          ...
          "AND",
          //["custrecord_lmry_code_account_sunat", 'noneof', "@NONE@"],"AND",
          ["subsidiary", "anyof", subsidiaries]
        ],
        columns: [
          'internalid',
          'displayname',
        ...
        myPage.data.forEach(function (result) {
          ...
          switch (type) {
            ...
            case "Bank":
              ...
              break;
            case "Expense":
              accountCategory = "rounding";
              break;
            case "Income":
              accountCategory = "roundingCredit";
              break;
            case "OthIncome":
              accountCategory = "roundingCredit";
              break;
          ...
          resultContext.push({ id, type, text : name, subsidiary, accountCategory });
    ...
    function getSearchInvoice(filterContext) {
      ...
      if (onlyAviable) {
        ...
        let pageDataBatch = dataBatch.runPaged({ pageSize: 1000 });
        if (pageDataBatch) {
          pageDataBatch.pageRanges.forEach(function (pageRange) {
            let myPage = pageDataBatch.fetch({ index: pageRange.index });
            myPage.data.forEach(function (result) {
              let status = result.getValue(result.columns[1]);
              let invoiceDetails = parseStringToObject(result.getValue(result.columns[0]));
              //It is not completed
              if (status != 3) {
    ...
    return {
      ...
      getSearchAccountBySubsidiary : getSearchAccountBySubsidiary
    }
