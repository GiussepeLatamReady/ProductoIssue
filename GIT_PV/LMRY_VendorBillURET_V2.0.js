...
define(['require', './Latam_Library/LMRY_UniversalSetting_Purchase_LBRY', 'N/record', 'N/runtime', 'N/search', 'N/log', 'N/config', 'N/https', 'N/url', 'N/ui/serverWidget',
  ...
  './Latam_Library/LMRY_BR_Redirect_Payments_LBRY_V2.0', './Latam_Library/LMRY_CO_Duplicate_Credit_Memos_LBRY_V2.0',"N/suiteAppInfo", './Latam_Library/LMRY_Validate_TranID_LBRY'
],
  ...
  function (require, library_Uni_Setting, record, runtime, search, log, config, https, url, serverWidget, library, library1, libraryDIOT, Library_WHT_Transaction, ST_Library_Transaction,
    ...
    Library_ExchangeRate, Library_RenameFields, Library_RedirecPayment, Library_Duplicate,suiteAppInfo,lbry_valTranID) {
      ...
      try {
        ...
        if (LMRY_Result[0] == "CO") {
          if (library.getAuthorization(720, licenses) == true) {
            if (context.type == 'edit') {
              var changeObj = Library_Duplicate.Verification_Duplicate(context, true);
        ...
        if(LMRY_Result[0] == 'DO' && library.getAuthorization(1111, licenses) == true && type_interface == 'CSVIMPORT'){
          lbry_valTranID.setReferenceNumberCSV(recordObj);
        }
    ...
    function createCOLineFieldsWHT(formObj, recordObj, type) {
      try {
        ...
        var filters = [
          ...
          "AND",
          ["custrecord_lmry_ntax_transactiontypes", "anyof", "4"],
          "AND",
        ...
        if (expenseSublist && JSON.stringify(expenseSublist) != "{}") {
          //RETECREE
          var expenseFieldCREE = expenseSublist.getField({
            id: 'custcol_lmry_co_autoretecree'
          });
  
          if (expenseFieldCREE && JSON.stringify(expenseFieldCREE) != "{}") {
          ...
          if (expenseFieldFTE && JSON.stringify(expenseFieldFTE) != "{}") {
          ...
          if (expenseFieldICA && JSON.stringify(expenseFieldICA) != "{}") {
          ...
          if (expenseFieldIVA && JSON.stringify(expenseFieldIVA) != "{}") {
