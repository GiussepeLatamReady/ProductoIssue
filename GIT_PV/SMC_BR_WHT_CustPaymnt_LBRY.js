...
define(['N/log', 'N/search', 'N/transaction', 'N/runtime', 'N/record', 'N/format', "N/util", '../Latam_Library/LMRY_libSendingEmailsLBRY_V2.0', "../Latam_Library/LMRY_Log_LBRY_V2.0"],
    function (log, search, transaction, runtime, record, format, util, library_mail, libLog) {
        ...
        var MEMO_WHT_2 = "(LatamTax - Withholding Tax)";
            ...
            if (form) {
                if (paymentTaxResults.length) {
                    ...
                    if (Object.keys(reclassAccounts).length && Object.keys(subtypeAccounts).length) {
                        ...
                        var aprilFirst2025 = new Date(2025, 3, 1);
                        log.error("aprilFirst2025",aprilFirst2025)
                        var formattedDate = format.format({
                            value: aprilFirst2025,
                            type: format.Type.DATE
                          });
                          log.error("formattedDate",formattedDate)
                        journalRecord.setValue('trandate', aprilFirst2025);
                        if (true) {
                            journalRecord.setValue("postingperiod", 1092);
                        }
                        ...
                        var whtLines = [];
                        log.error("paymentTaxResults",paymentTaxResults);
                        ...
                        log.error("journalRecord.getLineCount({ sublistId: line })",journalRecord.getLineCount({ sublistId: "line" }))
            ...
            var searchWHT = search.create({
                ...
                filters:
                    [
                        ...
                        [
                            ["applyingtransaction.memomain", "startswith", MEMO_WHT],
                            "OR",
                            ["applyingtransaction.memomain", "startswith", MEMO_WHT_2]
                        ], "AND",
