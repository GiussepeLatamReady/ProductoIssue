...
define(['./Latam_Library/LMRY_UniversalSetting_LBRY', './Latam_Library/LMRY_HideView3LBRY_V2.0',
    ...
    function (library_Uni_Setting, library_hideview3, Library_BoletoBancario, Library_Mail, Library_Number,
        ...
        libraryFleteGlobales, libBoTaxes, PE_STE_TaxLibrary) {
        ...
        function beforeLoad(scriptContext) {
            try {
                ...
                if (LMRY_Result[0] == "BO" && scriptContext.type == "copy" && Library_Mail.getAuthorization(827, licenses)) {
            ...
            try {
                ...
                if (scriptContext.type == 'create' || scriptContext.type == 'copy' || scriptContext.type == 'edit' || scriptContext.type == 'view') {
                    if (LMRY_Result[0] == "MX" && Library_Mail.getAuthorization(30, licenses)) {
                        ...
                        var numLines = RCD.getLineCount({ sublistId: 'item' });

                        for (var i = 0; i < numLines; i++) {
                            var itemType = RCD.getSublistValue({ sublistId: "item", fieldId: "itemtype", line: i }) || "";
                ...
                if (LMRY_Result[0] == "BO" && Library_Mail.getAuthorization(827, licenses)) {
                    if (scriptContext.type == "delete") {
            ...
            try {
                ...
                if (scriptContext.type == 'create' || scriptContext.type == 'edit' || scriptContext.type == 'copy') {
                    if (LMRY_Result[0] == 'MX' && (Library_Mail.getAuthorization(243, licenses) == true || Library_Mail.getAuthorization(972, licenses) == true)) {
                        if (Library_Mail.getAuthorization(672, licenses) == false) {
        ...
        function setWithholdingValues(country, recordObj, licenses) {
            ...
            if (swFeature == true && ['MAPREDUCE', 'SCHEDULED'].indexOf(runtime.executionContext) == -1) {
                ...
                for (var i = 0; i < numItems; i++) {
                    ...
                    if ((appliesLine == true || appliesLine == "T") && (taxLine == false || taxLine == "F")) {
                        if (!whtTaxCode) {
                            whtTaxCode = recordObj.getSublistValue({ sublistId: "item", line: i, fieldId: "custcol_4601_witaxcode" }) || "";
                        }
                        if (!whtTaxRate) {
                            whtTaxRate = recordObj.getSublistValue({ sublistId: "item", line: i, fieldId: "custcol_4601_witaxrate" }) || "";
                        }
                        ...
                        log.debug("whtBaseAmtLine", whtBaseAmtLine);
                ...
                if (whtAppliesTo == false || whtAppliesTo == "F" || !whtAppliesTo) {
                    whtTaxCode = "";
                    whtTaxRate = "";
                }

                if (Number(whtTaxCode)) {
                ...
                log.debug("[whtTaxCode,whtTaxName,whtTaxRate,whtTaxAmount,whtBaseAmount]", [whtTaxCode, whtTaxName, whtTaxRate, whtTaxAmount, whtBaseAmount].join(","));

                recordObj.setValue('custbody_lmry_wtax_code', whtTaxCode);
                recordObj.setValue('custbody_lmry_wtax_code_des', whtTaxName);
                recordObj.setValue('custbody_lmry_wtax_rate', whtTaxRate);
        ...
        function Crear_Wht_Rule(RCD_OBJ, scriptContext) {
            ...
            var numLines = RCD_OBJ.getLineCount({ sublistId: "item" });
            for (var i = 0; i < numLines; i++) {
                var itemType = RCD_OBJ.getSublistValue({ sublistId: "item", fieldId: "itemtype", line: i }) || "";
                var value = RCD_OBJ.getSublistValue({ sublistId: "item", line: i, fieldId: "custcol_lmry_wht_rule" });
                value = Number(value) || "";
